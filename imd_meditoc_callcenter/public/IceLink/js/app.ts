namespace chat.websync {
    export class App {
        ...
        private iceServers: fm.icelink.IceServer[] = [
            new fm.icelink.IceServer("stun:stun.frozenmountain.com:3478?transport=udp", "test", "pa55w0rd!"),
            //new fm.icelink.IceServer("turn:turn.frozenmountain.com:3478?transport=udp", "test", "pa55w0rd!"),
            //new fm.icelink.IceServer("turns:turn.frozenmountain.com:443?transport=udp", "test", "pa55w0rd!"),
            //new fm.icelink.IceServer("stun:stun.l.google.com:19302?transport=udp"),
            //new fm.icelink.IceServer("stun:stun1.l.google.com:19302?transport=udp"),
        ...
        private chromeExtensionInstallButton = document.getElementById(
        ...
        public startLocalMedia(
            ...
            audioDeviceList: HTMLSelectElement,
            videoDeviceList: HTMLSelectElement
        ): fm.icelink.Future<fm.icelink.LocalMedia> {
            ...
            try {
                ...
                fm.icelink.Plugin.install(pluginConfig).then(
                    (result) => {
                        ...
                        if (!fm.icelink.Plugin.isReady(true)) {
                            ...
                            if (fm.icelink.Plugin.isReady()) {
                                promise.reject(
                                    new Error("This browser supports WebRTC, but does not support media capture.")
                                );
                            } else {
                                promise.reject(
                                    new Error("This browser does not support WebRTC, and no plugin could be found.")
                                );
                            }
                            return;
                        }
                        ...
                        var audio = true;
                        var video = captureScreen
                            ? new fm.icelink.VideoConfig(window.screen.width, window.screen.height, 3)
                            : new fm.icelink.VideoConfig(640, 480, 30);
                        ...
                        this.localMedia.start().then(
                                ...
                                if (audioDeviceList) {
                                    ...
                                    localMedia.getAudioSourceInputs().then((inputs) => {
                                        for (var input of inputs) {
                                            ...
                                            option.selected =
                                                currentAudioSourceInput != null &&
                                                currentAudioSourceInput.getId() == input.getId();
                                    ...
                                    if (!captureScreen) {
                                        ...
                                        localMedia.getVideoSourceInputs().then((inputs) => {
                                            for (var input of inputs) {
                                                ...
                                                option.text = input.getName();
                                                option.selected =
                                                    currentVideoSourceInput != null &&
                                                    currentVideoSourceInput.getId() == input.getId();
                            ...
                            (ex) => {
                                if (
                                    captureScreen &&
                                    fm.icelink.Plugin.getChromeExtensionRequired() &&
                                    !fm.icelink.Plugin.getChromeExtensionInstalled()
                                ) {
                                    promise.reject(
                                        new Error(
                                            ex +
        ...
        public joinAsync(
            incomingMessage: (name: string, message: string, sending: false, recived: true) => void,
            peerLeft: (name: string) => void,
            peerJoined: (name: string) => void
        ): fm.icelink.Future<Object> {
        ...
        private autoSignalling(
            incomingMessage: (name: string, message: string, sending: false, recived: true) => void,
            peerLeft: (name: string) => void,
            peerJoined: (name: string) => void
        ): Signalling {
            var self = this;
            return new AutoSignalling(
                this.websyncServerUrl,
                this.name,
                this.sessionId,
                (remoteClient: any): fm.icelink.Connection => {
        ...
        private manualSignalling(
            incomingMessage: (name: string, message: string, sending: false, recived: true) => void,
            peerLeft: (name: string) => void,
            peerJoined: (name: string) => void
        ): Signalling {
            var self = this;
            return new ManualSignalling(
                this.websyncServerUrl,
                this.name,
                this.sessionId,
                (remoteClient: any): fm.icelink.Connection => {
        ...
        private connection(
            ...
            connection.addOnStateChange((c: fm.icelink.Connection) => {
                ...
                if (connection.getState() === fm.icelink.ConnectionState.Connected) {
                    ...
                    connection.getState() === fm.icelink.ConnectionState.Closing ||
                    ...
                    if (!self.SIGNAL_MANUALLY) self.signalling.reconnect(remoteClient, connection);
                }
            });
            return connection;
        }
        ...
        public toggleMute(mute: boolean): boolean {
            // Vídeo Mute
            var videoTrack = this.localMedia.getVideoTrack();
            videoTrack.setMuted(mute);
            // Audio Mute
            var audioTrack = this.localMedia.getAudioTrack();
            audioTrack.setMuted(mute);
            //return  Mute
            return audioTrack.getMuted() == videoTrack.getMuted() ? true : false;
        }
