...
class CallCenterController {
    constructor() {
        ...
        this.apiCancelarConsulta = "Api/CallCenter/Cancelar/Folio/Especialista/Consulta";
    ...
    async funcCancelarConsulta(entNuevaConsulta) {
        let response = { Code: 0, Message: "", Result: false };
        try {
            const apiResponse = await fetch(`${serverMain}${this.apiCancelarConsulta}`, {
                method: "POST",
                body: JSON.stringify(entNuevaConsulta),
                headers: MeditocHeadersCT,
            });
    ...
    async funcColaboradorOnline(entOnlineMod) {
        let response = { Code: 0, Message: "", Result: false };
        try {
            const apiResponse = await fetch(`${serverMain}${this.apiColaboradorOnline}`, {
                method: "POST",
    ...
    async funcCrearConsultaFolio(iIdColaborador = 0, sFolio = "", iIdUsuarioMod = 0) {
        let response = { Code: 0, Message: "", Result: {} };
        try {
            const apiResponse = await fetch(
    ...
    async funcIniciarConsulta(iIdConsulta = 0, iIdColaborador = 0, iIdUsuarioMod = 0) {
        let response = { Code: 0, Message: "", Result: false };
        try {
            const apiResponse = await fetch(
    ...
    async funcFinalizarConsulta(iIdConsulta = 0, iIdColaborador = 0, iIdUsuarioMod = 0) {
        let response = { Code: 0, Message: "", Result: false };
        try {
            const apiResponse = await fetch(
    ...
    async funcSaveHistorialClinico(entHistorialClinico) {
        ...
        try {
            ...
            response.Code = -1;
            response.Message = "Ocurrió un error al intentar guardar el historial clinico";
        }
        return response;
    }
