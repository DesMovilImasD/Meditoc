...
const {
  MeditocHeadersCT,
  MeditocHeaders,
} = require('../configurations/headersConfig')
...
class ColaboradorController {
  ...
  async funcSaveColaborador(entColaborador) {
    ...
    try {
      const apiResponse = await fetch(
        `${serverMain}${this.apiSaveColaborador}`,
        {
          method: 'POST',
  ...
  async funcGetColaboradores(
    ...
    try {
      ...
      response.Code = -1
      response.Message =
        'Ocurrió un error al intentar obtener los colaboradores'
    }
    return response
  }
  ...
  async funcSaveColaboradorFoto(piIdColaborador, piIdUsuarioMod, foto) {
    ...
    try {
      ...
      response.Code = -1
      response.Message =
        'Ocurrió un error al intentar guardar la foto del colaborador'
    }
    return response
  }
  ...
  async funcGetColaboradorFoto(piIdColaborador) {
    ...
    try {
      ...
      response.Code = -1
      response.Message =
        'Ocurrió un error al intentar obtener la foto del colaborador'
    }
    return response
  }
